...
truncate -s 0 crypto-config.yaml
echo "
OrdererOrgs:
  # ---------------------------------------------------------------------------
  # Orderer
  ...
  - Name: Orderer
    ...
    Specs:
      ...
      - Hostname: orderer5
      - Hostname: orderer6
      - Hostname: orderer7" >> crypto-config.yaml
echo "
...
do
	...
	echo "  # ---------------------------------------------------------------------------" >> crypto-config.yaml
	echo "  - Name: Org$OURNUM" >> crypto-config.yaml
	echo "    Domain: org$OURNUM.example.com" >> crypto-config.yaml
	echo "    EnableNodeOUs: true" >> crypto-config.yaml
	echo "    Template:" >> crypto-config.yaml
	echo "      Count: ${field[$i]}" >> crypto-config.yaml
...
truncate -s 0 docker-compose-cli.yaml
echo "
version: '2'

volumes:
  orderer.example.com:" >>docker-compose-cli.yaml
for i in $(seq 0 $(($ORGN-1)))
do
    ...
    do
       PEER=$(($z))
       OR=$(($i+1))
       echo "  peer$PEER.org$OR.example.com:" >> docker-compose-cli.yaml
    done
done
echo "
networks:
...
truncate -s 0 docker-compose-couch.yaml
echo " 
version: '2'
...
services:" >> docker-compose-couch.yaml
couchdb=0
PORT=5984
for i in $(seq 0 $(($ORGN-1)))
do
  ...
  peer$PEER.org$OR.example.com:
    ...
    depends_on:
      - couchdb$couchdb">> docker-compose-couch.yaml
        couchdb=$(($couchdb+1))
        PORT=$(($PORT+1000))
    done
done
...
changeInBase(){
truncate -s 0 base/docker-compose-base.yaml
echo "
...
truncate -s 0 docker-compose-e2e-template.yaml
PORT=7054
echo " 
version: '2'

volumes: 
  orderer.example.com:" >>docker-compose-e2e-template.yaml
...
do
    ...
    do
       PEER=$(($z))
       OR=$(($i+1))
       echo "  peer$PEER.org$OR.example.com:" >> docker-compose-e2e-template.yaml
    done
done
echo "
networks:
...
replacePrivateKey(){
truncate -s 0 byfn.sh
cp addbyfn.sh byfn.sh
# do in for
for m in $(seq 1 $ORGN )
do
	OURNUM=$(($m))
  ...
  PRIV_KEY=$(ls *_sk) \
  cd "$CURRENT_DIR" \
  sed $OPTS "s/CA'$OURNUM'_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml' byfn.sh
done
...
sed -i '/#######    Generating anchor peer update for Org1MSP   ##########/a \
  set -x \
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/Org'$OURNUM'MSPanchors$CHANNEL_NAME.tx -channelID $CHANNEL_NAME -asOrg Org'$OURNUM'MSP\
  res=$? \
  set +x \
  if [ $res -ne 0 ]; then \
...
scriptschanges(){
truncate -s 0 scripts/utils.sh
cp scripts/addutils.sh scripts/utils.sh
for m in $(seq 1 $ORGN )
do 
	OURNUM=$(($m))
	sed -i '/# This is a collection of bash functions used by different scripts/a \
PEER0_ORG'$OURNUM'_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/org'$OURNUM'.example.com/peers/peer0.org'$OURNUM'.example.com/tls/ca.crt' scripts/utils.sh
done
...
changeinutils(){
truncate -s 0 scripts/script.sh
cp scripts/addscript.sh scripts/script.sh
...
updateAnchorPeers 0 '$OURNUM'' scripts/script.sh
done
