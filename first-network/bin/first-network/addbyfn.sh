...
function networkUp() {
  ...
  if [ "${IF_COUCHDB}" == "couchdb" ]; then
    if [ "$CONSENSUS_TYPE" == "kafka" ]; then
      IMAGE_TAG=$IMAGETAG docker-compose -f $COMPOSE_FILE -f $COMPOSE_FILE_KAFKA -f $COMPOSE_FILE_COUCH up -d 5>&1
      docker ps -a
    elif  [ "$CONSENSUS_TYPE" == "etcdraft" ]; then
      IMAGE_TAG=$IMAGETAG docker-compose -f $COMPOSE_FILE -f $COMPOSE_FILE_RAFT2 -f $COMPOSE_FILE_COUCH up -d 5>&1
      docker ps -a
    else
    ...
    if [ "$CONSENSUS_TYPE" == "kafka" ]; then
      IMAGE_TAG=$IMAGETAG docker-compose -f $COMPOSE_FILE -f $COMPOSE_FILE_KAFKA up -d 5>&1 
      docker ps -a
    elif  [ "$CONSENSUS_TYPE" == "etcdraft" ]; then
      IMAGE_TAG=$IMAGETAG docker-compose -f $COMPOSE_FILE -f $COMPOSE_FILE_RAFT2 up -d 5>&1 
      docker ps -a
    else
      IMAGE_TAG=$IMAGETAG docker-compose -f $COMPOSE_FILE up -d 5>&1 
      docker ps -a
    fi
  fi
  if [ $? -ne 0 ]; then
  ...
  while true; do
      echo "if you want to write transaction press 1
if you want to read press 2"
      ...
      if [ $text = "1" ]; then
	    ...
	    echo " the twxt is $etext"
	    docker exec cli scripts/innvoke.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE $etext
      fi
      sleep 5
      if [ $text = "2" ]; then
 
 	   docker exec cli scripts/query.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE 
      fi
  done
